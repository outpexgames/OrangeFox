	...
	process: function(bot, msg, suffix) {
		...
		.then(function(response) {
			...
			if(matches.length > 0) {
				...
				var finalMessage = ':frame_photo: **Image**: ' + filepath + '\n:four_leaf_clover: **Name:** ' + name + '\n:four_leaf_clover: **Subject:** ' + subject + '\n:speech_left: **Comment:**\n' + htmlToText.fromString(comment) + '\n:link: **Link:** ' + link;
				...
				if (matches.length > 0) {
					...
					var finalMessage = ':frame_photo: **Image**: ' + filepath + '\n:four_leaf_clover: **Name:** ' + name + '\n:four_leaf_clover: **Subject:** ' + subject + '\n:speech_left: **Comment:** `' + htmlToText.fromString(comment) + '`\n:link: **Link:** ' + link;
					msg.channel.send(finalMessage);
				} else {
					msg.channel.send(':four_leaf_clover:  4chan: No matches found.');
				}
			}
		})
